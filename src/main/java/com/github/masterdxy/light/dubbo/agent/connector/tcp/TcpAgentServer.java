...
import com.github.masterdxy.light.dubbo.agent.connector.tcp.codec.LineDelimiterEncoder;
...
import static com.github.masterdxy.light.dubbo.agent.common.Constants.DELIMITER;
 ...
 * @author tomoyo
 */
public final class TcpAgentServer extends AbstractNetworkServer {

    private static final Logger LOG = LoggerFactory.getLogger(TcpAgentServer.class);

    private static final int CLOSE_CHANNEL_TIMEOUT = 5;


    /**
     * Initialize TcpAgentServer.
     *
     ...
     */
    private TcpAgentServer(String serviceName,
    ...
    private void bootStrap(int threadPoolSize, long threadKeepAliveSecs) throws Exception {
        ...
        bootstrap.setPipelineFactory(() -> {
            ...
            //换行分割
            pipeline.addLast("line", new LineDelimiterEncoder());
            //压缩
            ...
            pipeline.addLast("delimiter", new DelimiterBasedFrameDecoder(MAX_FRAME_LENGTH, DELIMITER));//超过max_frame_length 会抛出TooLongFrameException
            //解压缩
    ...
    /**
     * Creates a {@link Builder} for creating new instance of {@link TcpAgentServer}.
     *
    ...
    /**
     * Builder to help create the TcpAgentServer.
     */
    public static class Builder {
        ...
        /**
         * Returns the simple class name of the first caller that is different than the {@link TcpAgentServer} class.
        ...
        private static String getCallerClassName() {
            ...
            for (StackTraceElement element : Iterables.skip(Arrays.asList(Thread.currentThread().getStackTrace()), 1)) {
                if (!element.getClassName().startsWith(TcpAgentServer.class.getName())) {
        ...
        /**
         * @return instance of {@code TcpAgentServer}
         */
        public TcpAgentServer build() {
            ...
            return new TcpAgentServer(serviceName, bindAddress, bossThreadPoolSize, workerThreadPoolSize,
